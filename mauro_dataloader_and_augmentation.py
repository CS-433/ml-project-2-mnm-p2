...
import re
#PATHS OF FOLDERS ON GOOGLE DRIVE
...
NEW_DATA_FOLDER = '/content/drive/MyDrive/ml/data/training/training/NEWDATA/'
...
class DataLoaderSegmentation(data.Dataset):
    ...
    def __init__(self, folder_path,images_folder = 'images',label_folder = 'labels',with_filter = False, filter = None):
        ...
        #if with filter, keep all images that contain the word filter in their filename
        if with_filter:
    ...
    def next(self,batch_size = 1):
        if batch_size == 1:
          ...
          return res
    #return current epoch on data loader
    def get_epoch(self):
      return int(self.currentidx/len(self.img_files))

    #function that splits the dataloader into 2 dataloaders that are test and train
    def split_train_test( self,seed = 1 ,test_portion = 0.2):

      #patho of folder containg data
      folder_path = self.folder_path
      ...
      #make a copy of the path files
      copy_img_files = self.img_files
      copy_mask_files = self.mask_files

      #create array of indices and then shuffle
      index_array = list(range(len(copy_img_files)))
      random.Random(seed).shuffle(index_array)

      #shuffle paths in order of index array
      copy_img_files =  list(map(copy_img_files.__getitem__, index_array))
      copy_mask_files = list(map(copy_mask_files.__getitem__, index_array))

      #determine portion of test set
      quantity_test = int(len(copy_img_files)*test_portion)

      #split data paths int test and train
      test_set.img_files = copy_img_files[:quantity_test]
      test_set.mask_files = copy_mask_files[:quantity_test]
...
def load_test_images(img_folder):
    ...
    for file in files_img:
        ...
        img = transform.functional.pil_to_tensor(PIL.Image.open(img_path)).type(torch.FloatTensor)/255
        ls_img.append(img)
